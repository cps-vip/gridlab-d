...
int controller_dg::init(OBJECT *parent)
{
	...
	/* // FIXME: What the heck is this. Very clearly trying to set a flag on an uninit
	   //   pointer. This makes no sense, especially since obj is explicitly and
	   //   unconditionally set to nullptr further down.
	...
	*/
	...
	if (dgs != nullptr)
	{
		...
		for (unsigned int i = 0; i < dgs->hit_count; i++)
		{
...
TIMESTAMP controller_dg::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (first_run) // Final init items - namely deltamode supersecond exciter
	{
		if (deltamode_inclusive && enable_subsecond_models) // Still "first run", but at least one powerflow has completed (call init dyn now)
		{
			...
			for (unsigned int index = 0; index < dgs->hit_count; index++)
			{
...
SIMULATIONMODE controller_dg::inter_deltaupdate(unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	...
	for (int index = 0; index < dgswitchFound; index++)
	{
		...
		// double total_phase_P = pSwitchObjs[index].power_in.Re(); // Unused
		...
		// FIXME: Assuming we want to check the switch status == 1 && (out of limit checks), not
		//   what was here, which is status == 1 && limit check 1 || limit check 2
			 ...
			 (phase_A_P > 0 || phase_B_P > 0 || phase_C_P > 0)))
		{
			...
			if (!flag_switchOn || (controlTime == delta_time))
			{
				...
				// FIXME: Assume this is intended to be assignment, not equality check
