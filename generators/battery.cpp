	...
	if (!use_internal_battery_model)
	{
		...
		if (parent != nullptr)
		{
			if (gl_object_isa(parent, "meter", "powerflow"))
			{
				...
				else if (((phases & 0x03) == 0x03) || ((phases & 0x05) == 0x05) || ((phases & 0x06) == 0x06))
				{ // two-phase
				...
				else if (((phases & 0x01) == 0x01) || ((phases & 0x02) == 0x02) || ((phases & 0x04) == 0x04))
				{ // single phase
...
TIMESTAMP battery::sync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (!use_internal_battery_model)
	{
		...
		else
		{
			...
			if (!recalculate)
			{ // if forced recalculate is false, check where the time is
			...
			else
			{
				...
				{ // discharging
					...
					if ((Energy + (V_Internal * I_Prev.Re()).Re() * t2) <= margin)
					{ // headed to empty
				...
				{ // charging
					...
					if (Energy + ((V_Internal * I_Prev.Re()) * efficiency * t2).Re() >= (E_Max - margin))
					{ // if it is this far, it is charging at max
...
TIMESTAMP battery::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	{ // use_internal_battery_model is true
		// Pull the load
...
double battery::check_state_change_time_delta(unsigned int64 delta_time, unsigned long dt)
{
	...
	if (bat_load != 0)
	{
		...
		internal_battery_load = v_oc * bat_load / v_t;
		b_soc_reserve = pSocReserve->get_double();
		if (internal_battery_load <= 0) // TODO: check that adding equality here doesn't lead to problems
		{
			time_return = (delta_time) + (TIMESTAMP)ceil((b_soc_reserve - soc) * e_max * 3600 / internal_battery_load);
		}
		else // if (internal_battery_load > 0) // Before ln 2595 was <=, this covered other case. Now this can be else
		{
