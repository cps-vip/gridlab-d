 ...
 */
#ifdef __cplusplus
inline TRANSFORM *gl_transform_getfirst(void)
{
	return callback->transform.getnext(NULL);
};
...
#ifdef __cplusplus
inline randomvar_struct *gl_randomvar_getfirst(void)
{
	return callback->randomvar.getnext(NULL);
};
...
#define LOCKED(X, C)           \
	...
	} /**< @todo this is deprecated and should not be used anymore */
...
private: // internals
	...
	{
		if (buf)
			gld_core::wlock(&buf->lock);
	};
	inline void unlock(void)
	{
		if (buf)
			gld_core::wunlock(&buf->lock);
	};
...
private:
	...
	/// Constructor
public:
	inline gld_rlock(OBJECT *obj) : my(obj) { gld_core::rlock(&my->lock); };
	/// Destructor
public:
	inline ~gld_rlock(void) { gld_core::runlock(&my->lock); };
};
...
private:
	...
	/// Constructor
public:
	inline gld_wlock(OBJECT *obj) : my(obj) { gld_core::wlock(&my->lock); };
	/// Destructor
public:
	inline ~gld_wlock(void) { gld_core::wunlock(&my->lock); };
};
...
#define GL_STRUCT(T, X)                                                                                \
protected:                                                                                             \
	T X;                                                                                               \
                                                                                                       \
public:                                                                                                \
	static inline size_t get_##X##_offset(void) { return (char *)&(defaults->X) - (char *)defaults; }; \
	inline T get_##X(void)                                                                             \
	{                                                                                                  \
		gld_rlock _lock(my());                                                                         \
		return X;                                                                                      \
	};                                                                                                 \
...
#define GL_STRING(T, X)                                                                                \
protected:                                                                                             \
	T X;                                                                                               \
                                                                                                       \
public:                                                                                                \
	static inline size_t get_##X##_offset(void) { return (char *)&(defaults->X) - (char *)defaults; }; \
	inline char *get_##X(void)                                                                         \
	{                                                                                                  \
		gld_rlock _lock(my());                                                                         \
		return X.get_string();                                                                         \
	};                                                                                                 \
	...
	inline void set_##X(char *p)                                                                       \
	{                                                                                                  \
		gld_wlock _lock(my());                                                                         \
		strncpy(X, p, sizeof(X));                                                                      \
	};                                                                                                 \
	inline void set_##X(char *p, gld_wlock &) { strncpy(X, p, sizeof(X)); };                           \
	inline void set_##X(size_t n, char c)                                                              \
	{                                                                                                  \
		gld_wlock _lock(my());                                                                         \
		X[n] = c;                                                                                      \
	};                                                                                                 \
...
public:                                                                                                \
	...
	inline void set_##X(size_t n, T m)                                                                 \
	{                                                                                                  \
		gld_wlock _lock(my());                                                                         \
		X[n] = m;                                                                                      \
	};                                                                                                 \
...
public:                                                                                                \
	...
	inline T get_##X(gld_wlock &) { return X; };                                                       \
	inline void set_##X(T p) { X = p; };                                                               \
	inline void set_##X##_bits(T p)                                                                    \
	{                                                                                                  \
		gld_rlock _lock(my());                                                                         \
		(X) |= (p);                                                                                    \
	};                                                                                                 \
	inline void clr_##X##_bits(T p)                                                                    \
	{                                                                                                  \
		gld_rlock _lock(my());                                                                         \
		(X) &= ~(p);                                                                                   \
	};                                                                                                 \
...
public: // external accessors
	template <class T>
	inline void getp(PROPERTY &prop, T &value)
	{
		rlock();
		value = *(T *)(GETADDR(my(), &prop));
		wunlock();
	};
	template <class T>
	inline void setp(PROPERTY &prop, T &value)
	{
		wlock();
		*(T *)(GETADDR(my(), &prop)) = value;
		wunlock();
	};
	template <class T>
	inline void getp(PROPERTY &prop, T &value, gld_rlock &) { value = *(T *)(GETADDR(my(), &prop)); };
	template <class T>
	inline void getp(PROPERTY &prop, T &value, gld_wlock &) { value = *(T *)(GETADDR(my(), &prop)); };
	template <class T>
	inline void setp(PROPERTY &prop, T &value, gld_wlock &) { *(T *)(GETADDR(my(), &prop)) = value; };
...
public: // iterators
	...
	inline static gld_object *get_first(void)
	{
		...
		return OBJECTDATA(o, gld_object);
	};
...
public: // constructors/casts
	...
	inline gld_property(OBJECT *o, PROPERTYSTRUCT *p) : pstruct(nullpstruct), obj(o) { pstruct = *p; };
...
public: // special operations
	...
	template <class T>
	inline void getp(T &value, gld_rlock &) { value = *(T *)get_addr(); };
	template <class T>
	inline void getp(T &value, gld_wlock &) { value = *(T *)get_addr(); };
	template <class T>
	inline void setp(T &value, gld_wlock &) { *(T *)get_addr() = value; };
...
public:
	inline bool set_aggregate(char *spec, char *group)
	{
		aggr = callback->aggregate.create(spec, group);
		return aggr != NULL;
	};
	inline bool is_valid(void) { return aggr != NULL; };
	inline double get_value(void)
	{
		if (!aggr)
			throw "null aggregate";
		return callback->aggregate.refresh(aggr);
	};
};
...
#define EXPORT_INIT_C(X, C)                              \
	...
	{                                                    \
		...
		{                                                \
			if (obj != NULL)                             \
				return OBJECTDATA(obj, C)->init(parent); \
			else                                         \
				return 0;                                \
		}                                                \
		INIT_CATCHALL(X);                                \
...
#define EXPORT_PLC_C(X, C)                              \
	...
	{                                                   \
		try                                             \
		{                                               \
			return OBJECTDATA(obj, C)->plc(t1);         \
		}                                               \
		T_CATCHALL(plc, X);                             \
	}
