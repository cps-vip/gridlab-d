...
public:
	...
	{
		if (size <= max_val)
			exception(".set_max(%u): cannot shrink double_array", size);
		size_t r;
		...
		for (r = 0; r < max_val; r++)
		{
			if (x[r] != NULL)
			{
				...
				if (y == NULL)
					exception(".set_max(%u): unable to expand double_array", size);
	...
	void check_valid(const size_t r, const size_t c) const
	{
		if (!is_valid(r, c))
			exception(".check_value(%u,%u): invalid (r,c)", r, c);
	};
	...
	void dump(size_t r1 = 0, size_t r2 = -1, size_t c1 = 0, size_t c2 = -1)
	{
		...
		if (c2 == (size_t)-1)
			c2 = m - 1;
		if (r2 < r1 || c2 < c1)
			exception(".dump(%u,%u,%u,%u): invalid (r,c)", r1, r2, c1, c2);
		size_t r, c;
...
public:
	...
	{
		if (size <= max_val)
			exception(".set_max(%u): cannot shrink complex_array", size);
		size_t r;
		...
		for (r = 0; r < max_val; r++)
		{
			if (x[r] != NULL)
			{
				...
				if (y == NULL)
					exception(".set_max(%u): unable to expand complex_array", size);
...
typedef struct s_property_specs
