...
{
	// csv_reader *my = OBJECTDATA(obj, csv_reader); // Unused
...
int csv_reader::open(const char *file)
{
	...
	while (fgets(line, 1024, infile) != NULL)
	{
		...
		// size_t _len = strlen(line); // Unused
		if (line[0] == '#')
		{ // comment
			continue;
		}
		...
		else
		{
			...
			{ // read went 'backwards' or was blank, line discarded.
				;
			}
		}
	}
...
int csv_reader::read_prop(char *line)
{ // already pulled the '$' off the front
	// OBJECT *my = OBJECTHDR(this); // Unused
...
//			strcpy(guess_dt.tz, "GMT");
		...
		{ // somewhere between the last and the first element
	...
	if (sample_ct == 1)
	{ /* only one sample ~ ignore it and keep feeding the same data back, but in a year */
		next_ts += 365 * 24 * 3600;
		return -next_ts;
	}
