...
int tmy2_reader::open(const char *file)
{
	...
	if (fgets(buf, 500, fp))
	{
		...
		{
			// FIXME: need to provide default initialization, default to int default
			sscan_rv = 0;
		}
...
{
	if (city)
		strcpy(city, data_city); /* potential buffer overflow */
	if (state)
		strcpy(state, data_state); /* potential buffer overflow */
	if (degrees)
		*degrees = lat_degrees;
	if (minutes)
		*minutes = lat_minutes;
...
int tmy2_reader::read_data(double *dnr, double *dhr, double *ghr, double *tdb, double *rh, int *month, int *day, int *hour, double *wind, double *winddir, double *precip, double *snowDepth, double *pressure, double *extra_terr_dni, double *extra_terr_ghi, double *tot_sky_cov, double *opq_sky_cov)
{
	...
	if (tdb)
	{
		*tdb = ((double)tmp_tdb) / 10.0;
		if (*tdb < low_temp || low_temp == 0)
			low_temp = *tdb;
		else if (*tdb > high_temp || high_temp == 0)
			high_temp = *tdb;
	}
...
double tmy2_reader::calc_solar(COMPASS_PTS cpt, short doy, double lat, double sol_time, double dnr, double dhr, double ghr, double gnd_ref, double vert_angle = 90)
{
	...
	if (peak_solar == 0 || solar > peak_solar)
		peak_solar = solar;
	return solar;
}
/**
	...
	while (line < 8760 && file.next())
	{
		...
		if (hoy >= 0 && hoy < 8760)
		{
			...
			for (COMPASS_PTS c_point = CP_H; c_point < CP_LAST; c_point = COMPASS_PTS(c_point + 1))
			{
				...
				/* track records */
				if (sol_rad > record.solar || record.solar == 0)
					record.solar = sol_rad;
...
int climate::get_solar_for_location(double latitude, double longitude, double *direct, double *global, double *diffuse)
{
	...
	case CM_CUMULUS:
		...
		if (sol_z > RAD(90))
		{ // When sun is below the horizon, DNI must be zero.
			*direct = 0;
		}
		else
		{
...
void climate::update_cloud_pattern(TIMESTAMP delta_t)
{
	...
	if (row_shift != 0 || col_shift != 0)
	{
		...
		else if (row_shift >= 0 && col_shift <= 0)
		{ // Wind blows from SE to NW
			if (col_shift < 0)
			{
		...
		if (solar_zenith < (110 * PI / 180))
		{ // Only do these things if the sun is above (or slightly below) the horizon).
...
double climate::convert_to_binary_cloud()
{
	...
	do
	{
		...
		for (int i = CLOUD_TILE_SIZE; i < CLOUD_TILE_SIZE + on_screen_size; i++)
		{
			...
			{
				if (normalized_cloud_pattern[i][j] != EMPTY_VALUE && normalized_cloud_pattern[i][j] <= cut_elevation)
...
void climate::convert_to_fuzzy_cloud(double cut_elevation, int num_fuzzy_layers, double alpha)
{
	...
	if (cut_elevation == EMPTY_VALUE)
	{ // Initialization call uses EMPTY_VALUE as the cut elevation.
		// TDH: trivially parallelizable
	...
	for (int i = 0; i < num_fuzzy_layers; i++)
	{
		...
		for (int j = 0; j < cloud_pattern_size; j++)
		{
			...
			{
				// double binary = binary_cloud_pattern[j][kk]; // Unused variable
	...
	for (int j = 0; j < cloud_pattern_size; j++)
	{
		for (int k = 0; k < cloud_pattern_size; k++)
		{
			// double value = fuzzy_cloud_pattern[0][j][k]; // Unused
...
void climate::erase_off_screen_pattern(char edge_to_erase)
{
	...
	if (edge_to_erase == 'W')
	{
		...
		for (int i = 0; i < cloud_pattern_size; i++)
		{ // rows
			for (int j = 0; j < CLOUD_TILE_SIZE - 1; j++)
			{ // cols
				cloud_pattern[i][j] = EMPTY_VALUE;
			}
		}
	...
	else if (edge_to_erase == 'E')
	{
		...
		for (int i = 0; i < cloud_pattern_size; i++)
		{ // rows
			for (int j = cloud_pattern_size - CLOUD_TILE_SIZE + 1; j < cloud_pattern_size; j++)
			{ // cols
				cloud_pattern[i][j] = EMPTY_VALUE;
			}
		}
	...
	else if (edge_to_erase == 'N')
	{
		col_min = 0;
		row_min = 0;
		for (int i = cloud_pattern_size - CLOUD_TILE_SIZE + 1; i < cloud_pattern_size; i++)
		{ // rows
			for (int j = 0; j < cloud_pattern_size; j++)
			{ // cols
				cloud_pattern[i][j] = EMPTY_VALUE;
	...
	else if (edge_to_erase == 'S')
	{
		col_min = 0;
		row_min = 0;
		for (int i = 0; i < CLOUD_TILE_SIZE - 1; i++)
		{ // rows
			for (int j = 0; j < cloud_pattern_size; j++)
			{ // cols
				cloud_pattern[i][j] = EMPTY_VALUE;
...
{ // Used only for verification.
	...
	if (pattern == 'C')
	{
		for (int i = 0; i < (int)cloud_pattern.size(); i++)
		{
			for (int j = 0; j < (int)cloud_pattern.size(); j++)
			{
				if (j == ((int)cloud_pattern.size() - 1))
				{
	...
	{
		for (int i = 0; i < (int)binary_cloud_pattern.size(); i++)
		{
			for (int j = 0; j < (int)binary_cloud_pattern.size(); j++)
			{
				if (j == ((int)binary_cloud_pattern.size() - 1))
				{
...
int climate::calc_cloud_pattern_size(std::vector<std::vector<double>> &location_list)
{
	...
	{
		for (int i = 1; i < (int)location_list.size(); i++)
		{
...
TIMESTAMP climate::presync(TIMESTAMP t0) /* called in presync */
{
	...
	if (t0 > TS_ZERO && tmy != NULL)
	{
		...
		if (hoy < 0)
		{ // Taking care of the wrap-around at the year boundary.
			hoy = hoy + 8760;
		}
		switch (interpolate)
		{
	...
	if (t0 <= TS_ZERO)
		...
		if (reader_type == RT_CSV)
			...
			return TS_NEVER;
	else if (reader_type == RT_CSV)
		if (cloud_rv <= fabs(csv_rv))
			return cloud_rv;
		else
			return csv_rv;
	else if (cloud_rv <= fabs(tmy_rv))
		return cloud_rv;
	else
		return tmy_rv;
}
