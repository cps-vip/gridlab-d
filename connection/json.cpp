...
int json_translate(char *local, size_t local_len, char *remote, TRANSLATIONFLAG flag, size_t remote_len, ...)
{
	throw "obsolete code used";
	va_list ptr;
	va_start(ptr, remote_len);
...
JSONLIST *json::parse(char *buffer)
{
	...
	for (p = buffer; *p != '\0'; p++)
	{
		...
		case TAG0:
			...
			{
				state = TAG;
				t = tail->tag; /* TODO check tag length */
			}
			else
				goto Syntax;
			break;
		...
		case NUMBER: // same as above but without sign
			...
			else if (tail->type == JT_INTEGER && *p == '.')
			{
				*t++ = *p;
				tail->type = JT_REAL;
			}
