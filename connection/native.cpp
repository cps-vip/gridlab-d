...
extern "C" void outgoing_route_function(char *from, char *to, char *funcName, char *funcClass, void *data, size_t len)
{
	...
	{
		char buf[128];
		sprintf(buf, "native::outgoing_route_function: the relay function for function name %s could not be found.", funcName);
		throw(buf);
	}
	char message[3000] = "";
