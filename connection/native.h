...
{
public:
	GL_ATOMIC(enumeration, mode) // Macro expansion -> ;
...
private:
	...
	typedef enum
	{
		...
		_FIRST = INIT,
		_LAST = TERM, /* determine which are part of schema */
	} VARMAPINDEX;
	varmap *map[_NUMVMI];

public:
	inline connection_mode *get_connection(void) { return m; };
	inline void set_connection(connection_mode *p)
	{
		if (m)
			delete m;
		m = p;
	};
	inline void new_connection(const char *command)
	{
		if (m)
			delete m;
		m = connection_mode::new_instance(command);
	};
