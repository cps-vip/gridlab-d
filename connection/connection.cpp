...
int connection_mode::option(char *target, char *command)
{
	...
	if (strcmp(target, get_mode_name()) == 0)
	{
		...
		{
		case CM_CLIENT:
			return ((client *)this)->option(command);
		case CM_SERVER:
			return ((server *)this)->option(command);
		default:
			return 0;
		}
	}
...
{
	char temp[1024];
	// int len = sprintf(temp,"%lld",integer); // Unused
...
{
	int status;
	for (status = 0; status < (int)list->get_count(); status++)
	{
...
{
	int n;
	for (n = 0; n < (int)list->get_count(); n++)
	{
		cacheitem *item = list->get_item(n);
		// char *remote_name = item->get_remote(); // Unused
		// char *item_buffer = item->get_buffer(); // Unused
...
int connection_mode::server_response(int flag, ...)
{
	...
	case CM_SERVER:
		...
		if (count == 0)
		{
			...
			return send() == (int)transport->get_position();
		}
...
int connection_mode::client_initiated(int flag, ...)
{
	bool critical = flag == MSG_CRITICAL;
	int count;
	int msgcount = 0;
