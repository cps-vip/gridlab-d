...
helics_msg::helics_msg(MODULE *module)
{
	...
	if (!gl_publish_loadmethod(oclass, "configure", [](void *val, char *str) -> int
							   { return loadmethod_helics_msg_configure(static_cast<OBJECT *>(val), str); }))
			...
			if (message_type == HMT_GENERAL)
			{
					...
					if (pub.isValid())
					{
						...
						if (jReader->parse(config_info_temp.c_str(), config_info_temp.c_str() + config_length, &config_info, &parse_err))
						{
							if (config_info.isMember("message_type"))
							{
								...
								{
									char buf[128];
									snprintf(buf, 128, "The info field of the publication:%s defines an unknown message_type:%s. Valid message types are JSON and GENERAL", pub.getName(), individual_message_type.c_str());
									throw(buf);
								}
							}
							else
						...
						{
							char buf[128];
							snprintf(buf, 128, "Couldn't parse the HelicsPublication %s's info field. %s.", pub.getName(), parse_err.c_str());
							throw(buf);
						}
					}
				}
					...
					if (sub.isValid())
					{
						...
						if (jReader->parse(config_info_temp.c_str(), config_info_temp.c_str() + config_length, &config_info, &parse_err))
						{
							if (config_info.isMember("message_type"))
							{
								...
								{
									char buf[128];
									snprintf(buf, 128, "The info field of the subscription:%s defines an unknown message_type:%s. Valid message types are JSON and GENERAL", sub.getTarget(), individual_message_type.c_str());
									throw(buf);
								}
							}
							else
						...
						{
							char buf[128];
							snprintf(buf, 128, "Couldn't parse the HelicsSubscription %s's info field. %s.", sub.getTarget(), parse_err.c_str());
							throw(buf);
						}
					}
				}
				for (idx = 0; idx < ep_count; idx++)
				{
					...
					if (ep.isValid())
					{
						...
						{
							char buf[128];
							snprintf(buf, 128, "Couldn't parse the HelicsEndpoint %s's info field. %s.", ep.getName(), parse_err.c_str());
							throw(buf);
						}
					}
				}
...
int helics_msg::publishVariables()
{
	...
	for (vector<helics_value_publication *>::iterator pub = helics_value_publications.begin(); pub != helics_value_publications.end(); pub++)
	{
		...
		if ((*pub)->pObjectProperty->is_complex())
		{
			...
			// gld_unit *val_unit = (*pub)->pObjectProperty->get_unit(); // Unused
