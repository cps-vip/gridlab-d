	...
	for (p = text; true; p++)
	{
		...
		else if (*p == ',' || *p == ' ' || *p == ';' || *p == '\0')
		{
			...
			else if (*p == ';' || *p == '\0')
			{
				...
				for (d = 0; d < 8; d++)
					for (h = 0; h < 24; h++)
						if (days[(size_t)d] && hours[(size_t)h])
							SET_OCCUPIED(d, (size_t)h);
				if (*p == '\0')
					break;
				else
				{
...
int office::init(OBJECT *parent)
{
	...
	if (zone.hvac.cooling.capacity == 0)
		zone.hvac.cooling.capacity = oversize * (-zone.design.exterior_ua * (zone.hvac.cooling.design_temperature - zone.control.cooling_setpoint)																																																				  /* envelope */
												 - (zone.design.window_area[0] + zone.design.window_area[1] + zone.design.window_area[2] + zone.design.window_area[3] + zone.design.window_area[4] + zone.design.window_area[5] + zone.design.window_area[6] + zone.design.window_area[7] + zone.design.window_area[8]) * 100 * 3.412 * zone.design.glazing_coeff /* solar */
	...
	int i;
	for (i = 0; i < ((int)sizeof(map) / (int)sizeof(map[0])); i++)
	{
		if (map[i].test)
			throw map[i].desc;
	}
...
TIMESTAMP office::update_lighting(TIMESTAMP t1)
{
	...
	zone.lights.enduse.power.SetPowerFactor(zone.lights.capacity *
												zone.lights.fraction,
											zone.lights.enduse.power_factor, J);
	// energy calculation
	if (t0 > 0 && t1 > t0)
...
TIMESTAMP office::update_plugs(TIMESTAMP t1)
{
	...
	zone.plugs.enduse.power.SetPowerFactor(zone.plugs.capacity *
											   zone.plugs.fraction,
										   zone.plugs.enduse.power_factor, J);
	// energy calculation
	if (t0 > 0 && t1 > t0)
...
double office::update_hvac()
{
	...
	// const double &TbalHeat = zone.hvac.heating.balance_temperature; // Unused
